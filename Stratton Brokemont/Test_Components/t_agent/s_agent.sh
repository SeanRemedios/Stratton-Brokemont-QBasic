...
if [ -n "$1" ] # 1:Program
	...
	cd "Output"
	echo "===========================================================================" > agent_transaction.log
	echo "Agent Test Cases: #13-#18" >> agent_transaction.log
	...
	do
		...
		ext="${FILE_IN##*.}"
		if [ "txt" == "$ext" ] && [ "$FILE_IN" != "output.txt" ] && [ "$FILE_IN" != "transaction.txt" ] && [ "$FILE_IN" != "agent_transaction.log" ]
			then
			echo $FILE_IN
