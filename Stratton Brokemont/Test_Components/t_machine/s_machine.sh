...
if [ -n "$1" ] # 1:Program
	...
	do
		...
		if [ "txt" == "$ext" ] && [ "$FILE_IN" != "output.txt" ] && [ "$FILE_IN" != "transaction.txt" ] && [ "$FILE_IN" != "mach_transaction.txt" ]
			...
			for FILE_OUT in *
			do
				if [ "$FILE_OUT" != "mach_transaction.txt" ] && [ "$FILE_OUT" != "output.txt" ]
					then
					OUT="${FILE_OUT:1:1}"
					if [[ "$OUT" -eq "$INC" ]]
						then
						...
						if [ "$RESULT" != "" ]
							then
							echo "Test Case 0"$INC": FAILED" >> mach_transaction.txt
							sdiff "transaction.txt" "$FILE_OUT" >> mach_transaction.txt
							printf "\n" >> mach_transaction.txt
						else
							echo "Test Case 0"$INC": PASSED" >> mach_transaction.txt
						fi
					fi
				fi
			done
